...
let citiesThatEndWithI = cities.filter(city => {
...
/* 
    ...
    print "The fruit with index [THE ELEMENT INDEX] is an orange." If the color is not red or apple,
...
let ricePrice = foods.filter(food => food.tags.includes("rice")).map(food => food.price).reduce((total, price) => total + price)
